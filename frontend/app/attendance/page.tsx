    ...
    <div className="animate-fade-in">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-sm mb-1">Today — {formatDate(new Date().toISOString())}</p>
            <div className="flex items-center gap-3">
              ...
              {today?.check_in && <p className="text-slate-400 text-sm">In: {formatDateTime(today.check_in)}</p>}
              {today?.check_out && <p className="text-slate-400 text-sm">Out: {formatDateTime(today.check_out)}</p>}
